...
-  in the puudata. Use for- and -ifstatements. Place the results in a new column.
- Some help:
...
for (i in 1:nrow(data)) {
  if (data$PUULAJI[i] == 1) {
    ba <- append(ba, round((((data$LPM[i]*0.1)^2)*pi)/4,2))
...
while(i<nrow(data)){
  i=i+1
  if (data$PUULAJI[i]==1){
    ba2<- append(ba2, round((((data$LPM[i]*0.1)^2)*pi)/4,2))
  }
}
